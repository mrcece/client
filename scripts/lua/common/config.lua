...
local mmap = m.map
	...
	local beans = require_and_clear_loaded("cfg.beans")
...
local function LoadConfigs()
	local ctables = require_and_clear_loaded("cfg.tables")
...
function config.Init()
	...
	local p = tables.profession[1]
	print("###", p.id, p.class, p.name, p.t1, p.t2, tostring(p.t3))
end
